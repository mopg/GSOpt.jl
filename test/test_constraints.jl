...
@testset "Signomial Constraints" begin
    model = SPModel()
    @variable(model, x >= 0.1)
    @variable(model, y >= 0.1)
    @variable(model, z >= 0.1)

    constr9 = @constraint(model, x^2 + y^(-1) >= z^3) # this is signomial constraint
    @test constr9 isa GSOpt.SPConstraintRef
    @test is_valid(model, constr9)

    constr10 = @constraint(model, x + y <= z) # this is posynomial constraint
    @test constr10 isa GSOpt.SPConstraintRef
    @test is_valid(model, constr10)

    constr11 = @constraint(model, x^2 - y^(-1) <= z^3) # this is a signomial constraint
